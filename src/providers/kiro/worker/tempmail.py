#!/usr/bin/env python3
"""Temp-mail providers for kiro signup — pure HTTP, no browser.

Ported from grok-cli (xAI-specific) with AWS-specific markers and 6-digit OTP priority.
...
# ── OTP extraction (AWS-priority 6-digit) ────────────────────────────

# Primary: labeled 6-digit (AWS "verification code: 123456")
OTP_DIGIT6_RE = re.compile(
    r"(?:verification\s+code|confirmation\s+code|otp|one[- ]time"
    r"(?:\s+pass(?:word|code)?))[:\s#]*(\d{6})\b",
    re.I,
)
# Fallback: bare 6-digit with AWS context
OTP_DIGIT6_BARE_RE = re.compile(r"\b(\d{6})\b")
# Legacy: labeled hyphen code (cross-provider compatibility)
...
SKIP_DIGIT6 = {"111111", "222222", "123456", "000000", "999999", "666666"}
SKIP_HYPHEN = {
...
SKIP_LEGACY = {
    "signup", "verify", "account", "please", "gmail", "edge", "chrome",
    "safari", "webkit", "mozilla", "button", "submit",
...
AWS_MARKERS = (
    "signin.aws", "aws", "amazon", "amazon web services",
...
def _looks_like_ad(blob: str) -> bool:
    low = (blob or "").lower()
    return any(m in low for m in AD_MARKERS) and not any(m in low for m in AWS_MARKERS)


def _has_aws_context(blob: str) -> bool:
    low = (blob or "").lower()
    return any(m in low for m in AWS_MARKERS)


def extract_code(blob: str) -> str | None:
    """Extract OTP from email subject+body text.  None = no match/ad.

    Priority: labeled 6-digit (AWS primary) → labeled hyphen → bare 6-digit
    with AWS context → labeled legacy 6-char → bare hyphen with AWS context.
    """
    text = _decode_qpish(blob)
    if not text or _looks_like_ad(text):
        return None
    aws_context = _has_aws_context(text)

    # 1) Labeled 6-digit (primary — AWS verification codes)
    for m in OTP_DIGIT6_RE.finditer(text):
        code = m.group(1)
        if code not in SKIP_DIGIT6:
            return code

    # 2) Labeled hyphen code (cross-provider)
    for m in OTP_HYPHEN_RE.finditer(text):
        code = m.group(1).upper()
        if code.lower() not in SKIP_HYPHEN:
            return code

    # 3) Bare 6-digit with AWS context
    if aws_context:
        for m in OTP_DIGIT6_BARE_RE.finditer(text):
            code = m.group(1)
            if code not in SKIP_DIGIT6:
                return code

    # 4) Labeled legacy 6-char alnum
    ...
    # 5) Bare hyphen — AWS context required
    if aws_context:
...
class Zoromail:
    ...
    def create_account(self) -> str:
        ...
        username = "aws" + uuid.uuid4().hex[:10]
        data = self._api(
            "POST",
            "/emails",
...
class EmailBox:
    ...
    Provider preference list can be set via constructor or KIRO_TEMPMAIL_PROVIDERS
    ...
    def _providers_from_env() -> list[str] | None:
        raw = os.getenv("KIRO_TEMPMAIL_PROVIDERS", "")
        if not raw.strip():
            return None
