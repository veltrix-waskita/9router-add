...
# Inline AliyunCaptcha SDK (downloaded from o.alicdn.com) so DOMContentLoaded never
# blocks on a remote <script src> — the remote fetch hangs the goto when the CDN is
# slow/unreachable from the VPS. __SDK_INLINE__ is replaced per solve.
_SDK_PATH = Path(__file__).parent / "sdk.js"
_SDK_JS = _SDK_PATH.read_text() if _SDK_PATH.exists() else ""
...
async def _verify_in_page(page, tok: dict, scene_id: str, prefix: str) -> tuple:
    """Call VerifyCaptchaV3 from inside the page (same origin/proxy).

    Returns (VerifyCode, securityToken). The securityToken is the proof-of-pass
    that sites like pateway.ai consume (base64 {certifyId, sceneId, isSign,
    securityToken}) — without it the caller can't use the solve.
    """
    ...
    return result.get("VerifyCode", ""), result.get("securityToken", "")
...
async def solve_aliyun(scene_id: str, prefix: str, region: str = "sgp",
    ...
    page_html = (_PAGE_TMPL
                 ...
                 .replace("__SDK_INLINE__", _SDK_JS or "window.__sdkMissing=true;"))
    ...
    try:
        ...
        # The page URL is a synthetic host (never contacted — fulfilled locally);
        # default referer = the aliyun open endpoint itself, overridable per site.
        spoof_host = f"https://{prefix}.captcha-open-southeast.aliyuncs.com/"
        ref = referer or spoof_host

        async def _route(route):
            if route.request.url.endswith("/__solver"):
                await route.fulfill(status=200, content_type="text/html", body=page_html)
            else:
                await route.continue_(
                    headers={**route.request.headers, "referer": ref})

        await page.route("**/*", _route)

        async def load_once():
            await page.goto(f"{spoof_host}__solver",
        ...
        async def _attempt_puzzle(attempt, type_name):
            ...
            if raw:
                code, security_token = "T001", ""
            else:
                code, security_token = await _verify_in_page(page, token, scene_id, prefix)
            log.info("aliyun attempt %d [%s]: method=%s gap_x=%s dist=%.1f -> %s",
                     attempt, type_name, g.get("method", "cv2"), g["gap_x"], dist, code)
            return {"token": token, "verify_code": code, "security_token": security_token,
